...
const DataVerseFieldName = {
  ...
  DATE_STATUS_APPLIED: 'cre2c_datestatusapplied',
  DEADLINE_FOR_PI_RESPONSE: 'cre2c_datedeadlineforpiresponse',
...
this.formOnLoad = async (executionContext, section) => {
  ...
  this.hasPreviousOwnerOnChange(executionContext);
...
this.hasPreviousOwnerOnChange = executionContext => {
  ...
  const hasPreviousOwner = formContext.getAttribute(DataVerseFieldName.HAS_PREVIOUS_OWNER).getValue();

  const ownershipTab = formContext.ui.tabs.get('ownership_tab');

  const previousApplicantSection = ownershipTab.sections.get('previous_applicant_section');
  const previousOwnerSection = ownershipTab.sections.get('previous_owner_section');
